...
def main_page(model, mean_cv_accuracy, std_cv_accuracy, test_accuracy):
    ...
    if st.button('Prever Obesidade', key='predict_button'):
        if model is not None:
            ...
            # Adjust predict_proba to get probability of all classes for multi-class
            previsao_proba_all = model.predict_proba(novo_dado_df)

            # Display the prediction result based on the new mapping
            ...
            st.write(f"A previsão é: **{prediction_label.get(previsao[0], 'Desconhecido')}**")

            # Display probabilities for each class
            st.write("Probabilidades por Categoria:")
            for i, label in prediction_label.items():
                 # Ensure the index i exists in previsao_proba_all
                 if i < previsao_proba_all.shape[1]:
                    st.write(f"- {label}: **{previsao_proba_all[0, i]:.2f}**")
                 else:
                    st.write(f"- {label}: N/A (Categoria não prevista pelo modelo)")
            ...
            # Add recommendations based on the predicted obesity level
            st.subheader('Recomendações:')
            if previsao[0] == 3: # Obeso
                st.markdown("""
                ...
                *   Priorize o sono de qualidade, visando 7-9 hours por noite.
                *   Participe de grupos de apoio ou procure terapia para lidar com questões emocionais relacionadas à alimentação.
                """)
            elif previsao[0] == 2: # Sobrepeso
                 st.markdown("""
                *   Considere consultar um nutricionista para orientação sobre hábitos alimentares.
                *   Aumente a frequência ou intensidade da sua atividade física.
                *   Monitore a ingestão de calorias e o tamanho das porções.
                *   Evite o consumo excessivo de alimentos processados, açucarados e gordurosos.
                *   Beba bastante água ao longo do dia.
                 """)
            elif previsao[0] == 1: # Peso Normal
                 st.markdown("""
                ...
                *   Monitore seu peso e hábitos alimentares periodicamente para manter o peso saudável.
                *   Beba água suficiente ao longo do dia.
                *   Evite o consumo excessivo de alimentos processados e açucarados.
                 """)
            elif previsao[0] == 0: # Abaixo do Peso
                 st.markdown("""
                 *   Consulte um nutricionista para avaliar se sua ingestão calórica e nutricional é adequada.
                 *   Pode ser necessário aumentar a ingestão de calorias de forma saudável, focando em alimentos nutritivos.
                 *   Considere exercícios de fortalecimento muscular para ganhar massa.
                 *   Certifique-se de que está se alimentando em intervalos regulares.
                 """)
            else:
                 st.info("Não há recomendações específicas para este nível de previsão.")
